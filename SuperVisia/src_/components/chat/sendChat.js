...
import { getSendChat } from "./action";
  ...
  render() {
    return (
      <View >
        ...
        {(this.props.sendChat.loading || !this.props.sendChat.success) ? (
     ...
     <View>
       <ListView
      rightOpenValue={-75}
      dataSource={ds.cloneWithRows(this.props.sendChat.data)}
      renderRow={data =>
        <TouchableOpacity
        >
          <View style={styles.box}><Text>Welcome</Text></View><GBorder/></TouchableOpacity>
      }
    />
    </View>
    )
...
const mapDispatchToProps = dispatch => ({
  getSendChat: payload => dispatch(getSendChat(payload)),
});
